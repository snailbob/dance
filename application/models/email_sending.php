...
class Email_sending extends CI_Model
{
	...
	public function sendmail($info_arr,$other_info){	
		...
		$email_subj = $this->master_model->getRecords('outgoing_emails',array('file_name'=>$info_arr['view']));
		...
		$this->email->from($info_arr['from'],"Spacelli");
		...
		$this->email->message($this->load->view('email/'.$email_view,$other_info,true)); 
		$this->email->send();
		return true;
